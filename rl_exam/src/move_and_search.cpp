...
class SEARCH_OBJ{
    private:
		...
		struct Coord kuka, first_room, second_room, third_room;
...
SEARCH_OBJ::SEARCH_OBJ(){
		...
		first_room.x=-2.664177;
		first_room.y=-2.473649;
		first_room.w=1.0;

		second_room.x=3.985673;
		second_room.y=-2.668317;
		second_room.w=0.2;

		third_room.x=-2.318259;
		third_room.y=1.155192;
		third_room.w=-1.0;
	...
	center=false; //every time must be put to false before reaching the next center of the room
 	...
 	if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED){
		...
		if(goal.target_pose.pose.position.x==kuka.x && goal.target_pose.pose.position.y == kuka.y){
			...
			ros::ServiceClient client = n.serviceClient<rl_exam::service>("service");
